...
| 真实宿主用量接入（v0.9） | 本机 WorkBuddy 本地用量（可选开启） | 把宿主实测 Token/耗时导成账本草稿，skill 不再靠估算 |
...
```bash
# 只读本机 WorkBuddy 最近 7 天用量；无数据则提示跳过，不影响其它功能
...
- **不读平台计费 / 不联网 / 无密钥**：默认节省值来自你填的基准估计。v0.9 起**可选**开启「真实宿主用量接入」，仅**只读本机 WorkBuddy 的本地用量文件**（`~/.workbuddy/traces` 等）来补全 skill 实测成本；不调用任何平台计费 API、不发网络请求、不读任何密钥，满足 OpenClaw / 天禧 安全红线。无本机数据时该功能自动跳过，不影响其它功能。
