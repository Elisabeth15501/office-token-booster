...
- **推荐 Skill 的真实来源**：报告中的「推荐 Skill」由**内置静态规则表**按任务类型本地匹配产生，不联网获取；推荐条目的节省数字与来源链接均引自第三方项目的公开自述（README / benchmark），**未经本技能实测验证**，仅供参考。本技能不代为安装任何第三方 Skill，也不会生成任何安装命令——是否安装、如何安装由你自行判断。
